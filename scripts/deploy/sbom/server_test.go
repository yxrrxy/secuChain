...
func TestGenerateSBOM(t *testing.T) {
	...
	tests := []struct {
		...
		{Args{Language: "java", Format: "spdx", ProjectPath: "E:\\Github\\DomTok", ConfigPath: "./config.json", Token: "your-token"}, "SBOM generated successfully"},
		{Args{Language: "golang", Format: "cdx", ProjectPath: "E:\\Github\\DomTok", ConfigPath: "./config.json", Token: "your-token"}, "SBOM generated successfully"},
	}

	for _, tt := range tests {
		var reply string
		err = client.Call("SBOMService.GenerateSBOM", &tt.args, &reply)
		if err != nil {
			t.Fatalf("SBOMService.GenerateSBOM error: %v", err)
		}
		if reply != tt.expected {
			t.Errorf("Expected %q, got %q", tt.expected, reply)
		}
	}
}
...
func TestScanForVulnerabilities(t *testing.T) {
	...
	tests := []struct {
		...
		{Args{PackagePath: "E:\\Github\\DomTok", ConfigPath: "./config.json", Token: "your-token"}, "Vulnerability scan completed successfully"},
		{Args{PackagePath: "E:\\Github\\DomTok", ConfigPath: "./config.json", Token: "your-token"}, "Vulnerability scan completed successfully"},
	}
	for _, tt := range tests {
		var reply string
		err = client.Call("SBOMService.ScanForVulnerabilities", &tt.args, &reply)
		if err != nil {
